...
def setHits(toxic, organism):
    global hits
    global elegansHits
    global discoideumHits
    global drerioHits
    hits = getHits(toxic, organism)
    elegansHits = getHits(toxic, 'Elegans')
    discoideumHits = getHits(toxic, 'discoideum')
    drerioHits = getHits(toxic, 'd rerio')
        
def getHits(toxic, organism):
    if not toxic or not organism:
        hits = 0
        return hits
    else:
        t = toxic + ' ' + organism
        Entrez.email = 'A.N.Other@example.com'
        h = Entrez.esearch(db='pubmed', retmax=MAX_COUNT, term=t)
        result = Entrez.read(h)
        hits = result['Count']
        return hits
